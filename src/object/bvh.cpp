...
template<> BVH<Triangle>::Node::Node(const std::vector<uint32_t>& indices, const std::vector<Triangle>& triangles) : is_leaf(true), indices(indices)
{
  for (uint32_t i : indices)
  {
    ...
    {
      bounding_box.min = cm::min(bounding_box.min, triangles[i].get_triangle_vertex(v).pos);
      bounding_box.max = cm::max(bounding_box.max, triangles[i].get_triangle_vertex(v).pos);
    }
  }
}
