...
LightSampler::LightSampler(const std::vector<Light>& lights, const std::shared_ptr<Geometry> geometry) : lights(lights), geometry(geometry)
{
  ...
  for (uint32_t i = 0; i < object_instances.size(); i++)
  {
    ...
    if (instance.get_object().get_emissive_meshes().size() > 0) emissive_instance_indices.push_back(i);
  }
  ...
  for (uint32_t i = 0; i < emissive_instance_indices.size(); i++)
  {
    ...
    mesh_weights.resize(emissive_meshes.size(), 1);
    ...
    instance_distributions_entry.triangle_distributions_count = emissive_meshes.size();
...
LightSample LightSampler::sample(const HitInfo& hit_info, RandomGenerator& rnd) const
{
  ...
  if (random_num > -0.5f)
  {
    ...
    uint32_t triangle_index = triangle_distributions[instance_distributions_entry.triangle_distributions_offset + mesh_index].sample(rnd);
    ...
    const Triangle& triangle = instance.get_object().get_triangles()[mesh.triangle_index_offset + triangle_index];
    ...
    light_sample.emission = (geometry->get_material(mesh.material_id).get_emission(hit_info) * cos_theta) / glm::vec3(light_sample.pdf * distance * distance);
  }
  else
  {
