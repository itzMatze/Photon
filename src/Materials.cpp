...
bool Dielectric::scatter(const Ray& r_in, const RayPayload& rp, glm::vec4& attenuation, RandomGenerator* random_generator, Ray& scattered) const
{
    ...
    glm::vec3 reflected = glm::reflect(r_in.direction(), rp.normal);
    float ni_over_nt;
    ...
    if (glm::dot(r_in.direction(), rp.normal) > 0)
    {
        ...
        ni_over_nt = ref_idx;
        cosine = ref_idx * glm::dot(r_in.direction(), rp.normal) / glm::length(r_in.direction());
    }
    else
    {
        outward_normal = rp.normal;
        ni_over_nt = 1.0f / ref_idx;
        cosine = -glm::dot(r_in.direction(), rp.normal) / glm::length(r_in.direction());
    }
