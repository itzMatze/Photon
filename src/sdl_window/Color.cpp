...
uint32_t Color::get_hex_color() const
{
    ...
    uint32_t color = glm::clamp(static_cast<int>(values.a * 255), 0, 255);
    color <<= 8;
    color += glm::clamp(static_cast<int>(values.b * 255), 0, 255);
    color <<= 8;
    color += glm::clamp(static_cast<int>(values.g * 255), 0, 255);
    color <<= 8;
    color += glm::clamp(static_cast<int>(values.r * 255), 0, 255);
    return color;
}
